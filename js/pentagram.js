...
function draw() {
	...
	{ 
		delta *= -1;
		if (mode === RED) mode = BLUE; else
		if (mode === BLUE) mode = GREEN; else
...
function line(x0, y0, x1, y1, color) {
	...
	if (mode === BLUE || mode === GREEN) colorFunc = function() { return 255; };

	var pixelFunc;
	if (mode === RED || mode === BLUE) pixelFunc = function() { cv.putPixel(x0, y0, c); }; else
